...
namespace LobbyServer.Network
{
    ...
    public class LobbySession : Session
    {
        ...
        {
            var key = Blowfish.CalculateNetworkKey(time, seed);
        ...
        {
            var attribute = PacketManager.GetSubPacketHandlerInfo(subPacket);
            if (attribute == null)
                return true;
            ...
            {
                #if DEBUG
                    Console.WriteLine($"Rejecting packet ({subPacket.SubHeader.Type}, {subPacket.Handler}), lobby session ({Remote}) doesn't have Blowfish enabled!");
                #endif
                return false;
            }

            if ((attribute.Flags & SubPacketHandlerFlags.RequiresAccount) != 0 && ServiceAccount == null)
            {
                #if DEBUG
                    Console.WriteLine($"Rejecting packet ({subPacket.SubHeader.Type}, {subPacket.Handler}), lobby session ({Remote}) doesn't have assigned service account!");
                #endif
                return false;
            }
        ...
        {
            var sessionHash = SessionHash != 0u ? SessionHash : 0u;
