...
namespace Shared.Cryptography
{
    public class Blowfish
    {
        ...
        {
            Buffer.BlockCopy(pBase, 0, p, 0, pBase.Length);
            Buffer.BlockCopy(sBase, 0, s, 0, sBase.Length);
        ...
        public bool Encipher(byte[] data, int offset, int length)
        {
        ...
        public bool Decipher(byte[] data, int offset, int length)
        {
            if ((length - offset) % 8 != 0)
                return false;
        ...
        public static byte[] CalculateNetworkKey(uint time, byte[] seed)
        {
            using (var stream = new MemoryStream(0x2C))
            {
                using (var writer = new BinaryWriter(stream))
                {
                    writer.Write(0x12345678u);
                    writer.Write(time);
                    writer.Write(7000u); // based on expansion Heavensward - 3000, Endwalker - 6100, Dawntrail - 7000
                    writer.Write(seed);
                }

                return HashProvider.Md5(stream.GetBuffer());
            }
        }
    }
}
