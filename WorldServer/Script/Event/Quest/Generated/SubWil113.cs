// FFXIVTheMovie.ParserV3.11
// param used:
//_AGGRESSIVE_BNPC_HACK SET!!
//IGNORE_BNPCHACK_EVENTRANGE0 SET!!
using System.Numerics;
using Shared.Game;
using WorldServer.Game.Entity;
using WorldServer.Game.Event;

namespace WorldServer.Script.Quest
{
[EventScript(66157)]
public class SubWil113 : QuestScript
{

  //SEQ_0, 1 entries
  //SEQ_1, 5 entries
  //SEQ_2, 10 entries
  //SEQ_255, 1 entries

  //ACTOR0 = 1003929
  //ACTOR1 = 1003976
  //ACTOR2 = 1003971
  //ACTOR3 = 1003972
  //ACTOR4 = 1003973
  //ACTOR5 = 1003974
  //ACTOR6 = 1003975
  //ENEMY0 = 4102012
  //ENEMY1 = 4102013
  //EOBJECT0 = 2001868
  //EVENTRANGE0 = 4102016
  //EVENTACTIONRESCUEUNDERMIDDLE = 35
  //EVENTACTIONSEARCH = 1

  private const uint EVENT_ON_TALK = 0;
  private const uint EVENT_ON_EMOTE = 1;
  private const uint EVENT_ON_BNPC_KILL = 2;
  private const uint EVENT_ON_WITHIN_RANGE = 3;
  private const uint EVENT_ON_ENTER_TERRITORY = 4;
  private const uint EVENT_ON_EVENT_ITEM = 5;
  private const uint EVENT_ON_EOBJ_HIT = 6;
  private const uint EVENT_ON_SAY = 7;

  void onProgress(uint type, ulong param1, ulong param2, ulong param3 )
  {
    switch( quest.Sequence )
    {
      case 0:
      {
        if( type != EVENT_ON_BNPC_KILL ) Scene00000(); // Scene00000: Normal(QuestOffer, TargetCanMove), id=unknown
        // +Callback Scene00001: Normal(Talk, QuestAccept, TargetCanMove), id=ISEMBARD
        break;
      }
      case 1:
      {
        if( param1 == 1003976 ) // ACTOR1 = CHILD
        {
          if( quest.UI8AL != 1 )
          {
            Scene00002(); // Scene00002: Normal(Talk, TargetCanMove), id=CHILD
          }
          break;
        }
        if( param1 == 1003971 ) // ACTOR2 = unknown
        {
          Scene00003(); // Scene00003: Empty(None), id=unknown
          break;
        }
        if( param1 == 1003972 ) // ACTOR3 = unknown
        {
          Scene00004(); // Scene00004: Empty(None), id=unknown
          break;
        }
        if( param1 == 1003973 ) // ACTOR4 = unknown
        {
          Scene00005(); // Scene00005: Empty(None), id=unknown
          break;
        }
        if( param1 == 1003974 ) // ACTOR5 = unknown
        {
          Scene00007(); // Scene00007: Normal(Message, PopBNpc), id=unknown
          break;
        }
        break;
      }
      case 2:
      {
        if( param1 == 4102016 ) // EVENTRANGE0 = unknown
        {
          Scene00008(); // Scene00008: Normal(Message), id=unknown
          break;
        }
        if( param1 == 1003975 ) // ACTOR6 = OURCEN
        {
          if( !quest.getBitFlag8( 1 ) )
          {
            Scene00009(); // Scene00009: Normal(Talk, NpcDespawn, TargetCanMove), id=OURCEN
          }
          break;
        }
        // BNpcHack credit moved to ACTOR6
        if( param1 == 4102012 ) // ENEMY0 = unknown
        {
        // empty entry
          break;
        }
        if( param1 == 4102013 ) // ENEMY1 = unknown
        {
        // empty entry
          break;
        }
        if( param1 == 1003976 ) // ACTOR1 = CHILD
        {
          Scene00010(); // Scene00010: Normal(Talk, TargetCanMove), id=CHILD
          break;
        }
        if( param1 == 1003971 ) // ACTOR2 = unknown
        {
          Scene00011(); // Scene00011: Empty(None), id=unknown
          break;
        }
        if( param1 == 1003972 ) // ACTOR3 = unknown
        {
          Scene00012(); // Scene00012: Empty(None), id=unknown
          break;
        }
        if( param1 == 1003973 ) // ACTOR4 = unknown
        {
          Scene00013(); // Scene00013: Empty(None), id=unknown
          break;
        }
        if( param1 == 1003974 ) // ACTOR5 = unknown
        {
          Scene00014(); // Scene00014: Empty(None), id=unknown
          break;
        }
        if( param1 == 2001868 ) // EOBJECT0 = unknown
        {
          Scene00016(); // Scene00016: Empty(None), id=unknown
          break;
        }
        break;
      }
      case 255:
      {
        if( type != EVENT_ON_BNPC_KILL ) Scene00017(); // Scene00017: Normal(Talk, QuestReward, QuestComplete, TargetCanMove), id=ISEMBARD
        break;
      }
      default:
      {
        player.sendUrgent($"Sequence {quest.Sequence} not defined.  ");
        break;
      }
    }
  }

  public override void OnGossip(ulong actorId)
  {
    onProgress(EVENT_ON_TALK, actorId, 0, 0 );
  }

  public override void OnEmote(ulong actorId, ushort emoteId)
  {
    player.sendDebug($"emote: {emoteId}");
    onProgress(EVENT_ON_EMOTE, actorId, 0, emoteId );
  }

  public override void OnBNpcKill(BNpc bNpc)
  {
    onProgress(EVENT_ON_BNPC_KILL, bNpc.InstanceId, bNpc.BNpcNameId, 0);
  }

  public override void OnAreaTrigger(ulong actorId, WorldPosition position)
  {
    onProgress(EVENT_ON_WITHIN_RANGE, actorId, 0, 0 );
  }

  public override void OnEventTerritory()
  {
    onProgress(EVENT_ON_ENTER_TERRITORY, 0, 0, 0 );
  }
  public override void OnEventItem(ulong actorId)
  {
    onProgress(EVENT_ON_EVENT_ITEM, actorId, 0, 0 );
  }
  void checkProgressSeq0()
  {
    quest.Sequence = 1;
  }
  void checkProgressSeq1()
  {
    if( quest.UI8AL == 1 )
    {
      quest.UI8AL = 0 ;
      quest.setBitFlag8( 1, false );
      quest.Sequence = 2;
    }
  }
  void checkProgressSeq2()
  {
    if( quest.UI8AL == 2 )
    {
      quest.UI8AL = 0 ;
      quest.setBitFlag8( 1, false );
      quest.Sequence = 255;
    }
  }

private void Scene00000() //SEQ_0: , <No Var>, <No Flag>
  {
    player.sendDebug("SubWil113:66157 calling Scene00000: Normal(QuestOffer, TargetCanMove), id=unknown" );
    var callback = (SceneResult result) =>
    {
      if( result.numOfResults > 0 && result.GetResult( 0 ) == 1 )
      {
        Scene00001();
      }
    };
    owner.Event.NewScene( Id, 0, SceneFlags.HIDE_HOTBAR, Callback: callback );
  }
private void Scene00001() //SEQ_0: , <No Var>, <No Flag>
  {
    player.sendDebug("SubWil113:66157 calling Scene00001: Normal(Talk, QuestAccept, TargetCanMove), id=ISEMBARD" );
    var callback = (SceneResult result) =>
    {
      checkProgressSeq0();
    };
    owner.Event.NewScene( Id, 1, SceneFlags.HIDE_HOTBAR, Callback: callback );
  }

private void Scene00002() //SEQ_1: ACTOR1, UI8AL = 1, Flag8(1)=True(Todo:0)
  {
    player.sendDebug("SubWil113:66157 calling Scene00002: Normal(Talk, TargetCanMove), id=CHILD" );
    var callback = (SceneResult result) =>
    {
      quest.UI8AL =  (byte)( 1);
      quest.setBitFlag8( 1, true );
      player.SendQuestMessage(Id, 0, 0, 0, 0 );
      checkProgressSeq1();
    };
    owner.Event.NewScene( Id, 2, SceneFlags.HIDE_HOTBAR, Callback: callback );
  }

private void Scene00003() //SEQ_1: ACTOR2, <No Var>, <No Flag>
  {
    player.sendDebug("SubWil113:66157 calling Scene00003: Empty(None), id=unknown" );
  }

private void Scene00004() //SEQ_1: ACTOR3, <No Var>, <No Flag>
  {
    player.sendDebug("SubWil113:66157 calling Scene00004: Empty(None), id=unknown" );
  }

private void Scene00005() //SEQ_1: ACTOR4, <No Var>, <No Flag>
  {
    player.sendDebug("SubWil113:66157 calling Scene00005: Empty(None), id=unknown" );
  }

private void Scene00007() //SEQ_1: ACTOR5, <No Var>, <No Flag>
  {
    player.sendDebug("SubWil113:66157 calling Scene00007: Normal(Message, PopBNpc), id=unknown" );
    var callback = (SceneResult result) =>
    {
    };
    owner.Event.NewScene( Id, 7, SceneFlags.HIDE_HOTBAR, Callback: callback );
  }

private void Scene00008() //SEQ_2: EVENTRANGE0, <No Var>, <No Flag>
  {
    player.sendDebug("SubWil113:66157 calling Scene00008: Normal(Message), id=unknown" );
    var callback = (SceneResult result) =>
    {
      checkProgressSeq2();
    };
    owner.Event.NewScene( Id, 8, SceneFlags.HIDE_HOTBAR, Callback: callback );
  }

private void Scene00009() //SEQ_2: ACTOR6, UI8AL = 2, Flag8(1)=True(Todo:1)
  {
    player.sendDebug("SubWil113:66157 calling Scene00009: Normal(Talk, NpcDespawn, TargetCanMove), id=OURCEN" );
    var callback = (SceneResult result) =>
    {
      quest.UI8AL =  (byte)( 2);
      quest.setBitFlag8( 1, true );
      player.SendQuestMessage(Id, 1, 2, quest.UI8AL, 2 );
      checkProgressSeq2();
    };
    owner.Event.NewScene( Id, 9, SceneFlags.HIDE_HOTBAR, Callback: callback );
  }



private void Scene00010() //SEQ_2: ACTOR1, <No Var>, <No Flag>
  {
    player.sendDebug("SubWil113:66157 calling Scene00010: Normal(Talk, TargetCanMove), id=CHILD" );
    var callback = (SceneResult result) =>
    {
    };
    owner.Event.NewScene( Id, 10, SceneFlags.HIDE_HOTBAR, Callback: callback );
  }

private void Scene00011() //SEQ_2: ACTOR2, <No Var>, <No Flag>
  {
    player.sendDebug("SubWil113:66157 calling Scene00011: Empty(None), id=unknown" );
  }

private void Scene00012() //SEQ_2: ACTOR3, <No Var>, <No Flag>
  {
    player.sendDebug("SubWil113:66157 calling Scene00012: Empty(None), id=unknown" );
  }

private void Scene00013() //SEQ_2: ACTOR4, <No Var>, <No Flag>
  {
    player.sendDebug("SubWil113:66157 calling Scene00013: Empty(None), id=unknown" );
  }

private void Scene00014() //SEQ_2: ACTOR5, <No Var>, <No Flag>
  {
    player.sendDebug("SubWil113:66157 calling Scene00014: Empty(None), id=unknown" );
  }

private void Scene00016() //SEQ_2: EOBJECT0, <No Var>, <No Flag>
  {
    player.sendDebug("SubWil113:66157 calling Scene00016: Empty(None), id=unknown" );
    checkProgressSeq2();
  }

private void Scene00017() //SEQ_255: , <No Var>, <No Flag>
  {
    player.sendDebug("SubWil113:66157 calling Scene00017: Normal(Talk, QuestReward, QuestComplete, TargetCanMove), id=ISEMBARD" );
    var callback = (SceneResult result) =>
    {
      if( result.numOfResults > 0 && result.GetResult( 0 ) == 1 )
      {
        player.FinishQuest( Id, result.GetResult( 1 ) );
      }
    };
    owner.Event.NewScene( Id, 17, SceneFlags.HIDE_HOTBAR, Callback: callback );
  }
};
}
