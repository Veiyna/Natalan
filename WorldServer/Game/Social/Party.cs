...
namespace WorldServer.Game.Social
{
    public class Party : SocialBase
    {
        ...
        /// </summary>
        public List<Player> Players => Members.Select(m => MapManager.FindPlayer(m.Id)).Where(p => p != null).ToList();
        ...
        {
            ChatChannel = ChatChannelManager.CreateChatChannel(ChatChannelType.PartyChat);
        ...
        /// </summary>
        public override sealed void MemberAdd(Player invitee, bool broadcast)
        {
            ...
            if (Leader == null)
            {
                ...
                invitee.AddOnlineStatus(OnlineStatus.PartyLeader);
            }
                
            
            
            Members.Add(new Member(invitee, (byte)Members.Count));
            invitee.AddOnlineStatus(OnlineStatus.PartyMember);
            ChatChannelManager.AddToChannel(ChatChannel, invitee);
            if (broadcast)
            {
        ...
        private void MemberRemove(Member member)
        {
            ...
            leaver?.RemoveOnlineStatus(OnlineStatus.PartyMember);
            leaver?.RemoveOnlineStatus(OnlineStatus.PartyLeader);
            ChatChannelManager.RemoveFromChannel(ChatChannel, leaver);
        ...
        private void Disband()
        {
            ...
            onlineMembers.ForEach(m => m.RemoveOnlineStatus(OnlineStatus.PartyMember));
            onlineMembers.ForEach(m => m.RemoveOnlineStatus(OnlineStatus.PartyLeader));
        ...
        public void MemberPromote(Player promoter, string promoteeName)
        {
            ...
            promoter.RemoveOnlineStatus(OnlineStatus.PartyLeader);
            promotee.AddOnlineStatus(OnlineStatus.PartyLeader);
        ...
        public override void SendSocialList(Player recipient, ushort sequence)
        {
            ...
            //Own character always first
            foreach (Member member in Members.OrderByDescending(member => member.Id == recipient.Character.Id))
            {
        ...
        public void Send(SubPacket subPacket)
        {
