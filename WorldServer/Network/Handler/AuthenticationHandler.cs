...
using WorldServer.Network.Connection;
                ...
                switch (session.Channel)
                {
                    case ConnectionChannel.World:
                    {
                            ...
                            characters =>
                        {
                            ...
                            GameConnectionManager.GetConnection(helloWorld.ActorId).SetWorldSession(worldSession);
                        }));
                        break;
                    }
                    case ConnectionChannel.Chat:
                    {
                        ...
                        var chatSession = (ChatSession)session;
                        GameConnectionManager.GetConnection(helloWorld.ActorId).SetChatSession(chatSession);
                        break;
                    }
                }
            }));
